...
def wyswietl(sprawdz,):
    for i in range(len(sprawdz)):
        if sprawdz[i] == slowo[i]:
            textbox.insert(tk.END, sprawdz[i], 'pink')
            textbox.tag_config("pink", foreground="pink")
            rozwiazanie[i] = sprawdz[i]

        elif sprawdz[i] in slowo:
            textbox.insert(tk.END, sprawdz[i], 'red')
            textbox.tag_config("red", foreground="red")
        else:
            textbox.insert(tk.END, sprawdz[i])
    textbox.insert(tk.END,'\n', ("p"))
    if podane[0] == slowo and ilosc_prob[0]<11:
        rezultat.config(bg='yellow',text='Gratulacje,\n udało Ci się zgadnąć hasło\n za '+str(ilosc_prob[0])+' razem')
